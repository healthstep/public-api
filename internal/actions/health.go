...
func (c *HealthController) AdminListAnalyses(ctx context.Context, _ requests.AdminListAnalysesRequest) (responses.Response, int) {
	if !c.requireAdmin(ctx) {
		return &responses.ErrorResponse{Message: "forbidden"}, 403
	}
	resp, err := c.healthClient.AdminListAnalyses(ctx, &healthpb.AdminListAnalysesRequest{})
	if err != nil {
		return &responses.ErrorResponse{Message: "failed to list analyses"}, 500
	}
	return successData(resp.Analyses), 200
}

func (c *HealthController) AdminUpsertAnalysis(ctx context.Context, req requests.AdminUpsertAnalysisRequest) (responses.Response, int) {
	if !c.requireAdmin(ctx) {
		return &responses.ErrorResponse{Message: "forbidden"}, 403
	}
	pa := &healthpb.Analysis{
		Name:        req.Name,
	...
	resp, err := c.healthClient.AdminUpsertAnalysis(ctx, &healthpb.AdminUpsertAnalysisRequest{
		Analysis: pa,
	})
	if err != nil {
		return &responses.ErrorResponse{Message: "failed to upsert analysis"}, 500
	}
	return successData(resp.Analysis), 200
}

func (c *HealthController) AdminDeleteAnalysis(ctx context.Context, req requests.AdminDeleteAnalysisRequest) (responses.Response, int) {
	if !c.requireAdmin(ctx) {
		return &responses.ErrorResponse{Message: "forbidden"}, 403
	}
	resp, err := c.healthClient.AdminDeleteAnalysis(ctx, &healthpb.AdminDeleteAnalysisRequest{
		Id: req.ID,
	})
	if err != nil {
		return &responses.ErrorResponse{Message: "failed to delete analysis"}, 500
	}
	return successData(resp), 200
}
